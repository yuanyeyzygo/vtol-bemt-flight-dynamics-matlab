...
sim_init.pilot_stick = response_vector(cfg.response.pilot_stick, 4, 'cfg.response.pilot_stick');
sim_init.pilot_stick_to_control_gain_deg = response_vector( ...
    cfg.response.pilot_stick_to_control_gain_deg, 4, ...
...
if numel(vec) ~= expected_n || any(~isfinite(vec))
    error('RUN_RESPONSE_SIMULINK_SETUP:BadVector', ...
        '%s must contain %d finite values.', label, expected_n);
end
end

function cfg = merge_cfg_override(cfg, override)
if isempty(override)
    return;
end
...
for ii = 1:numel(fields)
    ...
    if isstruct(override.(name)) && isfield(cfg, name) && isstruct(cfg.(name))
        cfg.(name) = merge_cfg_override(cfg.(name), override.(name));
    else
