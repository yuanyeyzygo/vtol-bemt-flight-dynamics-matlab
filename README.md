...
| Need | Read this section | Main files |
|---|---|---|
| Pick the right entry script | [Entry-Point Map](#entry-point-map) | `RUN_ME.m`, `RUN_RESPONSE_SIMULINK_SETUP.m` |
| Run trim, stability derivatives, and control derivatives | [Quick Start](#quick-start), [MATLAB Individual-Blade](#1-matlab-individual-blade), [Trim Outputs](#trim-outputs) | `RUN_ME.m`, `RUN_TRIM_AND_STABILITY.m` |
| Choose `default`, `lookup`, or `excel` data | [Main Switches](#main-switches), [Lookup Data](#lookup-data), [Excel Aerodynamic Database](#excel-aerodynamic-database) | `RUN_ME.m`, `data_templates/` |
| Understand public default parameters | [Default Model](#default-model) | `RUN_ME.m`, `RUN_RESPONSE_SIMULINK_SETUP.m` |
| Run slow high-fidelity Simulink response | [Slow Simulink Individual-Blade](#2-slow-simulink-individual-blade), [Simulink Response](#simulink-response) | `CREATE_RESPONSE_SIMULINK_MODEL.m`, `VTOL_RESPONSE_SIMULINK.slx` |
| Run fast real-time-style Simulink response | [Fast Simulink Disk-Flap](#3-fast-simulink-disk-flap), [Simulink Response](#simulink-response) | `RUN_RESPONSE_SIMULINK_SETUP.m`, `VTOL_RESPONSE_SIMULINK_MEX.slx` |
...
| File | Use when | Rotor model | Data support | Main output |
|---|---|---|---|---|
| `RUN_ME.m` | You want to edit trim/stability settings directly in one interface file. | individual blade | `default`, `lookup`, `excel` for fuselage/controls | `trim_results` |
| `RUN_TRIM_AND_STABILITY.m` | You want a compact wrapper around the MATLAB trim and derivative workflow. | individual blade | follows `RUN_ME.m` style configuration | `trim_results` |
| `RUN_RESPONSE_SIMULINK_SETUP.m` | You want to trim first, then generate and initialize a Simulink response model. | individual blade or disk | `default`; detailed blade path can also use supported lookup/Excel data | base-workspace response variables and Simulink model |
| `RUN_FAST_DISK_MEX_RESPONSE.m` | You want to run the already generated fast Simulink/MEX disk model. | disk flap | public `default` path only | Simulink output `out` |
| `CREATE_RESPONSE_SIMULINK_MODEL.m` | You want to regenerate the slow individual-blade Simulink model explicitly. | individual blade | same setup data as the slow response workflow | `VTOL_RESPONSE_SIMULINK.slx` |
| `CREATE_RESPONSE_SIMULINK_MEX_MODEL.m` | You want to regenerate the fast disk/MEX Simulink model explicitly. | disk flap | public `default` path only | `VTOL_RESPONSE_SIMULINK_MEX.slx` |

The normal user path is `RUN_ME.m` for trim and derivative studies, and `RUN_RESPONSE_SIMULINK_SETUP.m` for response simulation.
...
cfg.response.control_delta              % optional direct-control debug increment
cfg.response.fixed_wing_control_delta   % optional fixed-control debug increment
...
The Simulink model contains a simple open-loop pilot-stick interface:

```text
Pilot stick input    = [collective, pitch, roll, yaw], normalized
Stick gain           = full-stick command scale in deg
Rotor tilt input     = six nacelle tilt angles in deg
```

The generated model does not include a closed-loop flight-control law. Users can replace the constant stick source with joystick hardware, a test signal, or another open-loop command source.
